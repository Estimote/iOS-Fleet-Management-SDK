...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    self.uidFilter = [[ESTEddystoneFilterUID alloc] initWithNamespaceID:ESTIMOTE_EDDYSTONE_NAMESPACE_ID];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    ESTEddystone *eddystone = [[self devicesForSection:indexPath.section] objectAtIndex:indexPath.row];
    
    if ([eddystone isKindOfClass:[ESTEddystoneURL class]])
    {
        ESTEddystoneURL *eddystoneURL = (ESTEddystoneURL *)eddystone;
        cell.textLabel.text = [NSString stringWithFormat:@"%@ / %@", eddystoneURL.macAddress, eddystoneURL.url];
    }
    else
    {
        cell.textLabel.text = [NSString stringWithFormat:@"%@", eddystone.macAddress];
    }
    
    if ([eddystone isKindOfClass:[ESTEddystoneTLM class]])
    {
        ESTEddystoneTLM *eddystoneTLM = (ESTEddystoneTLM *)eddystone;
        cell.detailTextLabel.text = [NSString stringWithFormat:@"RSSI: %zd Battery: %zd Temp: %.2f",
                                     [eddystoneTLM.rssi integerValue],
                                     [eddystoneTLM.batteryVoltage integerValue],
                                     [eddystoneTLM.temperature floatValue]];
    }
    else
    {
        cell.detailTextLabel.text = [NSString stringWithFormat:@"RSSI: %zd", [eddystone.rssi integerValue]];
    }
